...
from models.DIR_Net import Model as DIRModel
...
class Exp_Main(Exp_Basic):
    ...
    def _build_model(self):
        ...
        if self.args.model == 'DIR_Net':
            model = model_dict['DIR_Net'](self.args).float()
        else:
    ...
    def vali(self, vali_data, vali_loader, criterion, is_test = True):
        ...
        self.model.eval()
        with torch.no_grad():
            vali_pbar = tqdm(vali_loader, desc="Validation" if not is_test else "Testing", leave=False)
            for i, (batch_x, batch_y, batch_x_mark, batch_y_mark) in enumerate(vali_pbar):
                ...
                # If training, use ratio to scale the prediction
                if not is_test:
                ...
                vali_pbar.set_postfix({
                    'Loss': f'{loss.item():.7f}',
                    'Avg Loss': f'{np.average(total_loss):.7f}'
                })
    ...
    def train(self, setting):
        ...
        epoch_pbar = tqdm(range(self.args.train_epochs), desc="Training Epochs", position=0)
        for epoch in epoch_pbar:
            ...
            batch_pbar = tqdm(train_loader, desc=f"Epoch {epoch+1}/{self.args.train_epochs}", position=1, leave=False)
            ...
            epoch_pbar.set_postfix({
                'Train Loss': f'{train_loss:.7f}',
                'Vali Loss': f'{vali_loss:.7f}',
                'Test Loss': f'{test_loss:.7f}'
            })
    ...
    def test(self, setting, test=0):
        ...
        self.model.eval()
        with torch.no_grad():
            test_pbar = tqdm(test_loader, desc="Testing", position=0)
